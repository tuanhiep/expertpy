...
for ticker in tickers:
    ...
    if value_df['Portfolio_Value'].isna().any():
        print(f"Warning: There are NaN values in the 'Portfolio_Value' column for {ticker}.")
...
for idx, (ticker, result) in enumerate(results.items()):
    plt.plot(result['value_df'].index, result['value_df']['Portfolio_Value'], label=f'{ticker} Portfolio Value',
             color=colors[idx])
